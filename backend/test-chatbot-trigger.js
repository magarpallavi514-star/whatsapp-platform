...
async function testChatbotTrigger() {
  try {
    ...
    const account = await Account.findOne({ accountId: 'eno_2600003' });
    if (!account) {
      console.log('❌ Enromatics account not found');
      process.exit(1);
    }
    
    const accountId = 'eno_2600003'; // Enromatics account ID (String format used in KeywordRule)
    ...
    if (session) {
      ...
      console.log(`   Responses collected: ${Array.from(updatedSession.responses.entries()).map(([k,v]) => `${k}="${v}"`).join(', ')}`);
